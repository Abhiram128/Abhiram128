...
class Game:
    ...
    def play_sound(self, sound_name):
        ...
        if sound_name == "end":
            ...
            sound = pygame.mixer.Sound("end.wav")
    ...
    def bordercrash(self, x1, y1, x2, y2):
        for i in range(x2):
            if self.is_collision(x1, y1, i, 0):
                self.play_sound("end")
                raise "Collision Occured"
            if self.is_collision(x1, y1, i, y2):
                self.play_sound("end")
                raise "Collision Occured"
        for j in range(y2):
            if self.is_collision(x1, y1, 0, j):
                self.play_sound("end")
                raise "Collision Occured"
            if self.is_collision(x1, y1, x2, j):
                self.play_sound("end")
                raise "Collision Occured"
    ...
    def play(self):
        ...
        for i in range(2, self.snake.length):
            if self.is_collision(self.snake.x[0], self.snake.y[0], self.snake.x[i], self.snake.y[i]):
                self.play_sound("end")
                raise "Collision Occured"
        self.bordercrash(self.snake.x[0], self.snake.y[0], self.s_x, self.s_y)

    # self.border_crash(self.snake.x[0],self.snake.y[0],self.s_x,self.s_y)
    # ef border_crash(self,x1,y1,x2,y2):
    #  pass
